...
func (w *Wav) parseHeader() (bodyLength uint32, startPosition uint32, err error) {
	...
	if riffSize != uint32(len(w.wavByteData))-p {
		return 0, p, fmt.Errorf("error: this file maybe has been destoryed so that file length not equals flag value")
	}
	...
	var tmp uint32 = binary.BigEndian.Uint32((w.wavByteData)[p : p+4]) // 4 byte
	p += 4
	...
	p += 4
	var p_data_start = cksize
	var _ = p_data_start + 8

	var tmp_waveType = binary.LittleEndian.Uint16((w.wavByteData)[p : p+2]) // 2 byte，这个字段是小端存储
	p += 2
	waveType = int(tmp_waveType)
	if waveType != 1 {
	...
	var tmp1 = binary.BigEndian.Uint16((w.wavByteData)[p : p+2])
	p += 2
	...
	var dataSize = binary.LittleEndian.Uint32((w.wavByteData)[p : p+4]) // wav数据byte长度，小端存储
	p += 4
	if dataSize < 2 {
...
func (w *Wav) parseBody(startPosition uint32, bodyLength uint32) error {
	var p = startPosition
	var numSamples = bodyLength / uint32(w.blocklenSample) // 计算样本数
	...
	for i := 0; i < int(numSamples); i += 1 {
		for j := 0; j < w.Channels; j += 1 {
			...
			var tmp_sample int16
			binary.Read(bytesBuffer, binary.LittleEndian, &tmp_sample)
			p += uint32(w.bitNum / 8)
			w.Samples[j] = append(w.Samples[j], tmp_sample)
		}
	}
...
// packWave 打包wave部分
func (w *Wav) packWave() ([]byte, error) {
...
func (w *Wav) AppendBlank(milisecond uint32) {
	var zeroByteCount = milisecond * uint32(w.FrameRate/1000)
	for i := 0; i < w.Channels; i += 1 {
		var arr1 = make([]int16, zeroByteCount)
